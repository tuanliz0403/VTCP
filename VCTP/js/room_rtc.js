const APP_ID =  "b872025dd6db4832b5a01e42021e91a3"
...
let joinRoomInit = async () =>{
    ...
    getMembers()
    addBotMessageToDOm(`Chào mừng ${displayName} đến với phòng chat`)
...
let joinStream = async () => {
    ...
    localTracks = await AgoraRTC.createMicrophoneAndCameraTracks({}, {encoderConfig:{
        ...
        innerHeight:{min:480, ideal:1080, max:1080 }
    }})
...
let toggleScreen = async (e) => {
    ...
    cameraButton = document.getElementById('cam-btn')
    ...
    if(!screenIsSharing){
        ...
        for(let i = 0; videoFrames.length > i; i++){
            if(videoFrames[i].id != userIdInDisplayFrame.id){
...
let LeaveStream = async (e) => {
    ...
    channel.sendMesage({text:JSON.stringify({'type': 'user_left', 'uid':uid})})
}


document.getElementById('cam-btn').addEventListener('click', toggleCamera)
document.getElementById('mic-btn').addEventListener('click', toggleMic)
document.getElementById('scr-btn').addEventListener('click', toggleScreen)
